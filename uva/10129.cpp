...
void solve(){
    ...
    while(t--){
        int n;
        cin >> n;
        vector<vector<vector<string>>> g(26, vector<vector<string>>(26));
        vi id(26, 0), od(26, 0);
        UF uf(26);
        ...
        if(uf.cnt - (26 - st.size()) != 1){
            ...
            rep(i, 26){
                if(id[i] != od[i]){
                    v.push_back({i, id[i] - od[i]});
                }
            }
            #ifdef DEBUG
            cout << v.size() << endl;
            #endif
            if(!(v.size() == 0 or v.size() == 2)){
                cout << "The door cannot be opened." << '\n';
                continue;
            }
            else{
                if(v.size() == 2){
                    if(v[0].second + v[1].second == 0 and (v[0].second * v[1].second) == -1){
