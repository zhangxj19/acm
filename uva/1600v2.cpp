...
void solve(){
    ...
    while(t--){
        ...
        que.push({0, {0, 0, k}}); // dist, {x, y, remained step = k}
        const int inf = (int) 1e9;
        vector<vvi>  dist(m, vvi(n, vi(k+1, inf)));
        dist[0][0][k] = 0;
        while(!que.empty()){
            int d = que.top().first, x = que.top().second[0], y = que.top().second[1], rs = que.top().second[2]; que.pop();
            ...
            rep(i, 4){
                ...
                if(inbox(nx, ny)){
                    if(mp[nx][ny] == 0){
                        if(d + 1 < dist[nx][ny][k]){
                            dist[nx][ny][k] = d + 1;
                            que.push({dist[nx][ny][k], {nx, ny, k}});
                        }
                    }
                    else{
                        if(rs - 1 >= 0 and d + 1 < dist[nx][ny][rs-1]){
                            dist[nx][ny][rs-1] = d + 1;
                            que.push({dist[nx][ny][rs-1], {nx, ny, rs - 1}});
                        }
                    }
                }
            }
