...
int dfs(int x, int d){
    if(x == n) return 0;
    if(mem.count({x, d})) return mem[{x, d}];
    else{
        if(node[x].l + d <= node[x].ddl){
            //pick x
            int pick = node[x].p + dfs(x+1, d + node[x].l);
            //not pick x
            int notp = dfs(x+1, d);
            mem[{x,d}] = max(pick, notp);
        }
        else{
            // not pick x
            mem[{x,d}] = dfs(x+1, d);
...
void solve(){
    ...
    node.resize(n);
    rep(i, n){
        cin >> node[i].p >> node[i].l >> node[i].ddl;
    }
    sort(node.begin(), node.end(), [](Node &x, Node &y){
        return x.ddl < y.ddl;
    });
