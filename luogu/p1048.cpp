...
int dfs(int x, int timeremain){
    // if(x >= M) return;
    if(mem[x][timeremain] != -1) return mem[x][timeremain];


    if(x == M) return mem[x][timeremain] = 0;

    int re1=-1, re2=-1;
    if(timeremain - t[x] >= 0) re1 = dfs(x+1, timeremain - t[x]) + v[x];
    re2 = dfs(x+1, timeremain);

    return mem[x][timeremain] = max(re1, re2);
