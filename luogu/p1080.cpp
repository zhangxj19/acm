...
int readnum() {int x; cin >> x; return x;}
...
void solve(){
    ...
    sort(v.begin(), v.end(), [](pair<ll, ll>& p1, pair<ll, ll>& p2){
        return max(p2.second, p1.first*p1.second) < 
        max(p1.second, p2.first*p2.second);
    });
    ll re = 0, cnt = a;
    rep(i, n){
        re = max(re, cnt / v[i].second);
        cnt *= v[i].first;
    }
    #ifdef DEBUG
    each(x, v) cout << x.first << " " << x.second << ", ";
    cout << endl;
    #endif
